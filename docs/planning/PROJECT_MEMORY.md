...
| **ID**             | DEC-202511-001                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **Date**           | 2025-11-21                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Decision**       | **Phase 2 Feature Expansion: Add 11 Major Features + Citation Research Management System**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Rationale**      | Competitive analysis against Helperbird (52 features, 1M+ users) revealed AssisT has only 10 features with significant functional gaps in critical areas for students: annotations/note-taking, OCR for non-selectable text, translation for multilingual learners, dictionary lookup, and organized citation management. The NCAD citation research document identified that NO major citation tools (Zotero, Mendeley, EndNote) address accessibility needs for neurodivergent students. Decision made to implement 11 high-impact features plus comprehensive citation system inspired by NCAD research to create unique value proposition: accessibility-first citation management integrated with neurodivergent-friendly reading/writing tools. |
| **Alternatives**   | 1. **Incremental approach (2-3 features):** Rejected - insufficient to compete or meet student needs. 2. **Citation-only focus:** Rejected - students need reading/writing tools integrated with citations. 3. **Full Helperbird clone (52 features):** Rejected - unfocused, bloat risk, loss of education-specific differentiation. 4. **ADOPTED: Targeted 11 features + citation system:** Addresses critical gaps while maintaining educational focus.                                                                                                                                                                                                                                                                                            |
| **Impact**         | **Timeline:** 12-16 weeks across 4 phases. **Architecture:** Maintains monolithic content-simple.js with foundational features (Highlight Menu, Reading Mode, Keyboard Shortcuts), modular features in src/features/ (OCR, Dictionary, Annotations, Translation, Text Stats, Citations). **Storage:** Dual-mode for annotations (chrome.storage.local OR IndexedDB based on user choice), IndexedDB required for citations (large dataset). **Dependencies:** +5 npm packages (tesseract.js 2.5MB, @mozilla/readability 81KB, dexie 50KB, citeproc, open-graph-scraper). **Differentiation:** Accessibility-first citation management is UNIQUE - no competitor has this. **Rollback:** Git tag v0.1.0-pre-phase2 created for safety.                 |
| **Stakeholders**   | Lead Developer, Product Lead, Target Users (neurodivergent students), Future Contributors                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **Outcome/Action** | (1) Created git savepoint: v0.1.0-pre-phase2. (2) Updated package.json with dependencies. (3) Updated manifest.json with permissions: downloads, clipboardWrite, optional unlimitedStorage. (4) Phase 1 (Weeks 2-5): OCR, Highlight Menu, Reading Mode, Dictionary. (5) Phase 2 (Weeks 6-9): Annotations, Translation, Text Stats. (6) Phase 3 (Weeks 10-11): Font Library (+4 fonts), Keyboard Shortcuts. (7) Phase 4 (Weeks 12-16): Full Citation & Research Management System with Cite Them Right Harvard support.                                                                                                                                                                                                                                |

**Key Feature Decisions:**

1. **OCR:** Full system (Option B) - screenshot capture + PDF support + image OCR. High priority per user request.
2. **Annotations Storage:** Dual-mode - chrome.storage.local (default, 5MB) OR IndexedDB (unlimited). User dropdown choice in settings.
3. **Highlight Menu:** Foundational (in content-simple.js) - needs tight TTS/dictionary/translation integration.
4. **Translation API:** LibreTranslate (free) with option to switch to Google Translate API later. User choice.
5. **Reading Mode:** Foundational (in content-simple.js) - replaces entire DOM context, needs TTS preservation.
6. **Fonts:** Add 4 proven accessibility fonts: Lexend, Atkinson Hyperlegible, Arial, Verdana.
7. **Screenshot Tool:** Integrated with OCR (Option B) - region selection + full-page capture + text extraction.
8. **Keyboard Shortcuts:** Customizable system - user can remap all shortcuts, conflict detection vs Chrome/OS shortcuts.
9. **Dictionary API:** Free Dictionary API (dictionaryapi.dev) - zero config, no keys required.
10. **Implementation Order:** OCR → Highlight Menu → Reading Mode → Dictionary → Annotations → Translation → Text Stats → Fonts → Shortcuts → Citation System (5 sub-phases).

**Citation System Architecture:**

- **Metadata Extraction:** Multiple standards (OpenGraph, Dublin Core, JSON-LD, COinS) using open-graph-scraper.
- **Formatting:** CSL processor (citeproc-js) with harvard-cite-them-right-13th-edition.csl style.
- **NCAD Requirements:** Author-date format, "and" not ampersand, comma before page numbers, Day Month Year access dates, DOI handling per edition.
- **Organization:** Project-based system with visual cards (thumbnails), Kanban view (To Read/Reading/Cited), tags, folders.
- **Source Evaluation:** CRAAP test integration, DOI validation, retraction checks, predatory journal alerts.
- **Integration:** Google Docs toolbar, Canvas LMS editor button, export to Word/BibTeX/RIS/Zotero.
- **Accessibility:** Spell-check in metadata fields (top user request for dyslexic Zotero users), TTS for abstracts, Bionic Reading, focus mode for citation entry, full keyboard navigation.

**Risk Mitigation:**

- Savepoint tag enables rollback if any feature breaks extension.
- Each feature in separate implementation session with build + test cycle.
- Phase 1 features keep distance from existing TTS/dyslexia features.
- Storage strategy (dual-mode annotations) prevents quota issues.
- LazyLoad tesseract.js (2.5MB) only when OCR activated - avoid extension bloat.

**Related Documents:**

- Feature Comparison: FeaturePass_002/helperbird_analysis.md
- Citation Research: FeaturePass_002/citation.md
