...
| **ID**             | DEC-202510-022                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Date**           | 2025-10-30                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Decision**       | Adopt Incremental Strangler Fig Pattern for Safe Modularization of Content Script                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Rationale**      | **LEARNING FROM PAST FAILURES:** Previous modularization attempts (DEC-202510-020) failed due to "big-bang" refactoring approach - trying to convert the entire monolithic content-simple.js (3,034 lines) to ES6 modules simultaneously. This new approach uses the **Strangler Fig Pattern**: incrementally extracting one feature at a time while keeping the extension functional throughout. Key insights from comprehensive dependency analysis: (1) 13 distinct feature groups identified with varying coupling levels, (2) showToast() is called 41+ times across ALL features (must extract first), (3) Core TTS is foundational with 9+ dependent features (must extract LAST), (4) 6 features are completely isolated (safe to extract early), (5) Quiz Helper is tightly coupled to Canvas (extract together). **Why this will succeed where previous attempts failed:** (a) Incremental approach keeps extension working at each step, (b) Low-risk features extracted first to build confidence and validate patterns, (c) Build infrastructure (Vite) set up BEFORE any extraction begins, (d) Each extraction is a single commit with immediate testing, (e) Abstraction layers created before deleting old code, (f) Clear 10-week phased plan with defined success criteria. |
| **Alternatives**   | 1. **Keep monolithic forever:** Rejected - current file is unmaintainable at 3,034 lines and will only grow. 2. **Big-bang refactor again:** Rejected - already failed 3 times, documented in DEC-202510-020. 3. **Wait for Chrome to fix ES6 module support:** Rejected - platform constraints exist today, must work within them. 4. **Use different bundler:** Considered but Vite chosen for speed and MV3 Chrome Extension support.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Impact**         | **Development Time:** 10-week incremental plan (vs. 90 minutes wasted in previous failed attempts). **Risk Mitigation:** Extension remains functional throughout refactoring. **Maintainability:** Each feature becomes independently testable module. **Onboarding:** New developers can understand one feature module without reading 3,000 lines. **Testing:** Can unit test each module in isolation. **Performance:** No change - same functionality, better organization. **Future Development:** Can add new features as modules without touching existing code.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **Stakeholders**   | Lead Developer, AI Assistant (Claude), QA Team, Future Contributors                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Outcome/Action** | (1) Created comprehensive dependency map identifying 13 feature groups, 74 functions, 42 state variables, and coupling metrics. (2) Defined 6-phase extraction plan: Phase 1 (Utilities), Phase 2 (Isolated Features), Phase 3 (Moderately Coupled), Phase 4 (LMS Integrations), Phase 5 (High-Coupling), Phase 6 (Core TTS). (3) Prioritized extraction order by risk: Text Customization (lowest risk) → Quiz Helper & Core TTS (highest risk). (4) Will document progress in PROJECT_MEMORY.md after each successful extraction. (5) **Critical Success Criteria:** After each extraction, run `npm run build`, reload extension, test feature in Chrome - if it works, commit; if broken, revert and debug before proceeding.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |

**Related Documentation:**

- Dependency Map: Created by Explore agent (2025-10-30)
- Extraction Order: 13 features ranked by coupling score
- Previous Failure Analysis: DEC-202510-020 (why big-bang failed)
- Incremental Protocol: DEC-202510-021 (one-change-at-a-time rule)

**10-Week Phased Plan:**

**Phase 1: Core Utilities (Week 1)** - LOWEST RISK

1. Extract showToast() → `/src/core/ui/toast.js` (update 41+ imports)
2. Extract hexToRgba() → `/src/core/utils/color.js` (update 2 imports)
3. Create SettingsManager → `/src/core/storage/settings-manager.js` (pub/sub pattern)

**Phase 2: Isolated Features (Weeks 2-3)** - LOW RISK 4. Extract Text Customization → `/src/features/text-customization/` (0 dependencies) 5. Extract Screen Overlay → `/src/features/screen-overlay/` (1 dependency: toast) 6. Extract Dyslexia Mode → `/src/features/dyslexia-mode/` (1 dependency: toast)

**Phase 3: Moderately Coupled (Weeks 4-5)** - MEDIUM RISK 7. Extract STT → `/src/features/stt/` (1 dependency: toast) 8. Extract Reading Guide + Focus Mode → `/src/features/reading-aids/` (mutual exclusivity, extract together)

**Phase 4: LMS Integrations (Weeks 6-7)** - MEDIUM RISK 9. Extract Moodle → `/src/features/lms-integrations/moodle/` (2 dependencies: TTS, toast) 10. Extract Google Classroom → `/src/features/lms-integrations/google-classroom/` (2 dependencies: TTS, toast) 11. Extract Canvas → `/src/features/lms-integrations/canvas/` (2 dependencies: TTS, toast; 1 dependent: Quiz Helper) 12. Extract Read Page → `/src/features/read-page/` (2 dependencies: TTS, toast)

**Phase 5: High-Coupling Features (Week 8)** - HIGH RISK 13. Extract Quiz Helper → `/src/features/quiz-helper/` (3 dependencies: TTS, toast, Canvas adapter - tightly coupled)

**Phase 6: Core TTS Refactor (Weeks 9-10)** - HIGHEST RISK 14. Extract Core TTS → `/src/core/tts/` as shared module: - `/src/core/tts/reader.js` (readText function) - `/src/core/tts/highlighting.js` (highlightElement, highlightWordByWord, etc.) - `/src/core/tts/state.js` (currentUtterance, settings, synth, etc.) - `/src/core/tts/voice-manager.js` (loadVoices, voice selection)

**Build Infrastructure:**

- Use Vite (fast, MV3-compatible, ES6 module support)
- Configure for Chrome Extension with proper manifest handling
- Set up watch mode for development
- Maintain existing `npm run build` command

**Testing Strategy:**

- After each extraction: Build → Reload extension → Test feature → Commit if working
- Unit tests for extracted modules
- E2E tests for critical user flows
- Regression testing: All existing features must continue working

**Rollback Strategy:**

- Each extraction is a single git commit
- If extraction breaks extension, immediately `git revert HEAD`
- Fix issues in isolation before re-attempting
- Never batch multiple extractions in one commit

**Success Metrics:**

- Extension works after each extraction (zero downtime)
- Each feature passes unit tests in isolation
- All E2E tests continue passing
- Build time remains under 10 seconds
- No regressions in existing functionality

**Critical Rules:**

1. Never extract multiple features simultaneously
2. Build and test after EVERY extraction
3. Commit immediately after successful test
4. If broken, revert and debug before proceeding
5. Extract low-risk features first to validate patterns

**This Decision Supersedes:** DEC-202510-020 (which prohibited modularization entirely). The key difference: DEC-202510-020 failed because it attempted instant conversion with bundlers. This approach uses incremental extraction with proper build infrastructure, keeping the extension functional throughout.
