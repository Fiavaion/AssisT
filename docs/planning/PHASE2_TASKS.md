...
- [ ] CWS promo tile (440×280 static image)
- [ ] CWS submission — complete store listing, upload zip + screenshots + video URL
...
- [x] D.9: Create STT_USER_GUIDE.md (added Session 033)
...
## Phase 2 Extension: Bug Fixes

**Status**: In Progress
**Current**: Session 083 (2026-02-28)

### BugHive Bugs: AI Model System (#8–#13)

**Status**: `[x]` All Fixed — Session 083 (2026-02-28)

- [x] Bug #8 — AI model not available (Sonnet/Opus errors): Updated model IDs to 4.6 (`claude-sonnet-4-6`, `claude-opus-4-6`) across `claude-client.js`, `cloud-router.js`, all 9 feature files, `model-dropdown.js`
- [x] Bug #9 — Token limit cutoff + wrong model version: Doubled token limits (summarization 150-500→400-1500, textSimplification 400-800→800-1500, studyPath 2000→3000)
- [x] Bug #10 — Cloud AI showing "Local AI" in Knowledge Graph: Fixed `GRAPH_DEFAULT_CLOUD_MODEL = 'local'` → `'sonnet-4.6'`
- [x] Bug #11 — Study path missing AI indicator + text legibility: Added AI source to header meta; fixed tips CSS (solid `#f1f8e9` bg + `#2e7d32` text, WCAG AA); increased maxTokens
- [x] Bug #12 — Model identifiers inconsistent: Created shared `src/utils/ai-badge.js` utility with unified CSS classes (`assist-ai-badge--cloud`, `--local`, `--fallback`)
- [x] Bug #13 — Features not respecting provider/model choice: `ai-badge.js` reads both `cloudProvider` and `cloudModel` — badges now show "☁️ OpenAI: gpt-4o" style labels

**Files Created**: `src/utils/ai-badge.js` (~180 lines)
**Files Modified**: `claude-client.js`, `cloud-router.js`, `model-dropdown.js`, 9 feature files (~200 lines changed)
