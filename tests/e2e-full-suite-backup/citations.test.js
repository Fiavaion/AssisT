...
test.describe('Citation Accessibility', () => {
  test('should have proper ARIA labels on citation controls', async ({ popupPage }) => {
    ...
    const citationToggle = popupPage.locator('[data-testid="citation-toggle"]');
    await expect(citationToggle).toHaveAttribute('aria-label', 'Enable citations');

    // Check Save Citation button aria-label exists
    const saveCitationButton = popupPage.locator('[data-testid="save-citation-button"]');
    await expect(saveCitationButton).toHaveAttribute('aria-label', 'Save citation');

    // Check Citation Manager button aria-label exists
    const managerButton = popupPage.locator('[data-testid="citation-manager-button"]');
    const managerLabel = await managerButton.getAttribute('aria-label');
  ...
  test('should have proper aria-expanded on expand button', async ({ popupPage }) => {
    ...
    // Enable citations using JS
    await popupPage.evaluate(() => {
      const checkbox = document.querySelector('[data-testid="citation-toggle"]');
    ...
    // Click to expand using JS to ensure handler fires
    await expandButton.scrollIntoViewIfNeeded();
    await popupPage.evaluate(() => {
      const btn = document.querySelector('[data-testid="expand-citations-button"]');
      if (btn) btn.click();
    });
    await popupPage.waitForTimeout(500);

    // Now expanded - check actual state
    const isExpanded = await expandButton.getAttribute('aria-expanded');
    // Accept either expanded state or verify button is clickable (test might vary based on panel implementation)
...
test.describe('Citation Section Layout', () => {
  test('should have proper section structure', async ({ popupPage }) => {
    ...
    // Should have toggle control (main section element)
    const toggleControl = citationSection.locator('.toggle-control');
    await expect(toggleControl).toBeAttached();
  });
  ...
  test('should have description text when enabled', async ({ popupPage }) => {
    ...
    // Description should exist and contain expected text (feature-description class)
    const description = popupPage.locator('#citation-options-container .feature-description');
