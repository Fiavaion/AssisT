...
/**
 * Helper to toggle a checkbox via JavaScript (for custom toggle switches)
...
test.describe('Highlight Menu - Settings Configuration', () => {
  ...
  test('should toggle TTS button visibility', async ({ popupPage }) => {
    ...
    // Uncheck TTS button using JS helper
    await setCheckboxState(popupPage, '#highlight-menu-show-tts', false);
    await popupPage.waitForTimeout(100);
    expect(await ttsToggle.isChecked()).toBe(false);

    // Check TTS button again
    await setCheckboxState(popupPage, '#highlight-menu-show-tts', true);
    await popupPage.waitForTimeout(100);
    expect(await ttsToggle.isChecked()).toBe(true);
  });
...
test.describe('Highlight Menu - Feature Visibility', () => {
  ...
  test('should hide options when Highlight Menu disabled', async ({ popupPage }) => {
    ...
    // Disable Highlight Menu using JS helper
    await setCheckboxState(popupPage, '#highlight-menu-enabled', false);
    await popupPage.waitForTimeout(300);
  ...
  test('should toggle all button options', async ({ popupPage }) => {
    ...
    for (const toggleId of buttonToggles) {
      ...
      // Uncheck toggle using JS helper
      await setCheckboxState(popupPage, toggleId, false);
      await popupPage.waitForTimeout(50);
      expect(await toggle.isChecked()).toBe(false);

      // Check toggle again
      await setCheckboxState(popupPage, toggleId, true);
      await popupPage.waitForTimeout(50);
      expect(await toggle.isChecked()).toBe(true);
    }
  });
...
test.describe('Highlight Menu - Accessibility', () => {
  ...
  test('should have accessible button toggles', async ({ popupPage }) => {
    ...
    const buttonToggles = [
      { id: 'highlight-menu-show-tts', label: 'read aloud' },
    ...
    for (const button of buttonToggles) {
      ...
      await expect(label).toBeVisible();
      const labelText = await label.textContent();
      expect(labelText.toLowerCase()).toContain(button.label);
    }
  });
