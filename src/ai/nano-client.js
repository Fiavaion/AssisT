 ...
 *
 * NOTE: This is Chrome's on-device Gemini Nano (window.ai), NOT the Google Gemini REST API.
 * For the cloud Google Gemini API, see google-client.js.
 *
 * Requirements:
...
export async function checkGeminiAvailability() {
  try {
    ...
    console.error('[NanoClient] Availability check failed:', error);
    return {
      available: false,
      status: 'error',
...
export async function generateText(prompt, options = {}) {
  ...
  try {
    ...
    console.log('[NanoClient] Session created, generating text...');
    ...
    console.log('[NanoClient] Generation successful');
    return result;
  } catch (error) {
    console.error('[NanoClient] Generation failed:', error);
    throw new Error(`Gemini Nano generation failed: ${error.message}`);
  }
}

/**
 * Gets the Nano client interface
 *
