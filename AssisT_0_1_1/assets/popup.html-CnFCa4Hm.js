...
import { g as getStorageAdapter, s as sanitizeHTML, b as attachInteractiveHandler, e as attachDelegatedHandler, p as purify, l as loadShortcuts, S as SHORTCUT_LABELS, f as saveShortcuts, h as eventToShortcut, v as validateShortcut } from "./storage-adapter-C0pzedCF.js";
(function polyfill() {
...
class CitationManagerPanel {
  ...
  renderCitationList() {
    ...
    list.innerHTML = sanitizeHTML(
      this.filteredCitations.slice(0, 20).map((citation, index2) => this.renderCitationCard(citation, index2)).join("")
    );
    ...
    attachDelegatedHandler(list, ".citation-card", "Citation Card", (e, card) => {
      const index2 = Array.from(list.querySelectorAll(".citation-card")).indexOf(card);
      if (index2 >= 0) {
...
class PopupController {
  ...
  async setupPermissionBanner() {
    ...
    try {
      ...
      if (hasAllUrls) {
        ...
        const isRecent = justGranted && Date.now() - justGranted < 6e4;
        if (isRecent) {
  ...
  async ensureContentScriptLoaded() {
    ...
    const isLmsSite = url.includes("instructure.com") || url.includes("canvas.com") || url.includes("moodle.org") || url.includes("moodlecloud.com") || url.includes("classroom.google.com") || url.includes("docs.google.com");
    if (isLmsSite) {
  ...
  applyVisibilitySettings() {
    ...
    toggleSection("citation-section", "show_citations", false);
    toggleSection("citation-options-container", "show_citations", false);
    ...
    const isFeatureVisible = (visibilityKey, defaultValue = true) => {
      return visibility[visibilityKey] !== void 0 ? visibility[visibilityKey] : defaultValue;
    };
    ...
    const accordionConfig = {
      reading: { alwaysVisible: true },
      // TTS has no visibility toggle
      writing: {
        features: [{ key: "show_stt", default: false }]
      },
      lookup: { alwaysVisible: true },
      // Translation has no visibility toggle
      display: {
        features: [
      ...
      "ai-assist": { alwaysVisible: true }
      // AI mode has no visibility toggle
    };
  ...
  setupMinimizeClutterButton() {
    ...
    this.attachInteractiveHandler(btn, "Minimize Clutter Toggle", () => {
      ...
      chrome.storage.local.set({
        textStatsBadgeVisible: newState ? false : this.settings.ui_overlay.show_text_stats_badge !== false,
        textStatsNotificationsEnabled: newState ? false : this.settings.ui_overlay.show_text_stats_notifications !== false,
        featureNotificationsEnabled: newState ? false : this.settings.ui_overlay.show_feature_notifications !== false,
        tokenCounterVisible: newState ? false : this.settings.ui_overlay.show_token_counter !== false,
        readingProgressBadgeVisible: newState ? false : this.settings.ui_overlay.show_reading_progress_badge !== false
      });
      if (this.currentTab?.id) {
  ...
  updateLayoutModeUI(descriptionEl, previewEl, mode) {
    ...
    if (previewEl) {
      previewEl.innerHTML = sanitizeHTML(
        config.sections.map(
          (section) => `<span style="background: #e2e8f0; padding: 4px 8px; border-radius: 4px; font-size: 11px;">${section.icon} ${section.name}</span>`
        ).join("")
      );
    }
  }
...
Note: Dynamic section reorganization will be fully implemented in a future update. For now, you can use Organize Mode to manually reorder and hide sections.`
      );
    }
    ...
    chrome.storage.local.get("assist_profiles", (result) => {
      ...
      if (profile && profile.description) {
        ...
        const defaultDescriptions = {
          ...
          "Night Study": "Reduced eye strain with screen overlay, warm colors, and Pomodoro timer for study sessions.",
  ...
  setupAITab(modal) {
    ...
    if (apiKeyInput) {
      ...
      apiKeyInput.addEventListener("change", async () => {
        const provider2 = modal.querySelector("#cloud-provider").value;
        const { saveSecureAPIKey } = await __vitePreload(async () => {
          const { saveSecureAPIKey: saveSecureAPIKey2 } = await import("./secure-key-storage-C4xryL8P.js");
          return { saveSecureAPIKey: saveSecureAPIKey2 };
        }, true ? [] : void 0);
        const success = await saveSecureAPIKey(provider2, apiKeyInput.value);
    ...
    if (testBtn) {
      this.attachInteractiveHandler(testBtn, "API Key Test Button", async () => {
        const apiKeyInput2 = modal.querySelector("#cloud-api-key");
        const provider = modal.querySelector("#cloud-provider").value;
        await this.testAPIKey(provider, apiKeyInput2.value, modal);
      });
    }
  ...
  async loadApiKey(modal, provider) {
    ...
    try {
      const { getSecureAPIKey } = await __vitePreload(async () => {
        const { getSecureAPIKey: getSecureAPIKey2 } = await import("./secure-key-storage-C4xryL8P.js");
        return { getSecureAPIKey: getSecureAPIKey2 };
      }, true ? [] : void 0);
  ...
  async testAPIKey(provider, apiKey, modal) {
    ...
    try {
      const { saveSecureAPIKey, isValidKeyFormat, testSecureConnection } = await __vitePreload(async () => {
        const { saveSecureAPIKey: saveSecureAPIKey2, isValidKeyFormat: isValidKeyFormat2, testSecureConnection: testSecureConnection2 } = await import("./secure-key-storage-C4xryL8P.js");
        return { saveSecureAPIKey: saveSecureAPIKey2, isValidKeyFormat: isValidKeyFormat2, testSecureConnection: testSecureConnection2 };
      }, true ? [] : void 0);
...
ollama pull ${modelName}`
      ...
      if (taskType === "code") {
        ...
        if (otherModels.length > 0) {
          ...
          otherModels.forEach((model) => {
            ...
            option2.value = model.name;
            option2.textContent = model.name;
            otherGroup.appendChild(option2);
          });
  ...
  loadModalSettings() {
    ...
    loadCheckbox("show-citations", "show_citations");
  ...
  saveModalSettings() {
    ...
    saveCheckbox("show-citations", "show_citations");
    ...
    this.saveSettings();
    chrome.storage.local.set({
      textStatsBadgeVisible: this.settings.ui_overlay.minimize_clutter ? false : this.settings.ui_overlay.show_text_stats_badge !== false,
      textStatsNotificationsEnabled: this.settings.ui_overlay.minimize_clutter ? false : this.settings.ui_overlay.show_text_stats_notifications !== false,
      featureNotificationsEnabled: this.settings.ui_overlay.minimize_clutter ? false : this.settings.ui_overlay.show_feature_notifications !== false,
      tokenCounterVisible: this.settings.ui_overlay.minimize_clutter ? false : this.settings.ui_overlay.show_token_counter !== false,
      readingProgressBadgeVisible: this.settings.ui_overlay.minimize_clutter ? false : this.settings.ui_overlay.show_reading_progress_badge !== false
    });
  ...
  setupMagnifyingLens() {
    ...
    scaleSlider.value = this.settings.magnifyingLens.scale || 2;
  ...
  showVoiceCommandsModal() {
    ...
    this.attachInteractiveHandler(
      closeBtn,
      "Close Voice Commands Modal Button",
      () => modal.remove()
    );
...
//# sourceMappingURL=popup.html-CnFCa4Hm.js.map
